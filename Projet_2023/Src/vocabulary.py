...
class TrapeziumModality(Modality):
    ...
    def __eq__(self, other):
        return self.minSupport == other.minSupport and self.minCore == other.minCore and self.maxCore == other.maxCore and self.maxSupport == other.maxSupport
...
class Partition:
    """This class represents the partition of an attribute with several modalities, ex: 'age' = { 'young', 'medium', \
    'old' }"""
    ...
    def getDistance(self, mod1, mod2):
        ...
        if self.isTrapeziumPartition():
            minSupports = sorted([(mod, mod.minSupport) for mod in self.modalities.values()], key=lambda x: x[1])
            n = len(minSupports)
            mod1_obj = self.getModality(mod1)
            mod2_obj = self.getModality(mod2)
            d1 = 0
            d2 = 0
            index = minSupports.index((mod1_obj, mod1_obj.minSupport))
            while minSupports[index][1] != mod2_obj.minSupport:
                d1 += 1
                index = (index + 1) % n
            index = minSupports.index((mod1_obj, mod1_obj.minSupport))
            while minSupports[index][1] != mod2_obj.minSupport:
                d2 += 1
                index = (index - 1) % n
            return d1 / (self.nbModalitites - 1) if d1 < d2 else d2 / (self.nbModalitites - 1)
